...
export default function KnowledgeBaseConfig() {
  ...
  const [vectorDbConfigModified, setVectorDbConfigModified] = useState<
    Record<string, boolean>
  >({}); // 标记哪些字段被修改过
  const [vectorDbConfigExists, setVectorDbConfigExists] = useState<
    Record<string, boolean>
  >({}); // 标记哪些敏感字段已存在（但不存储值）
  ...
  // AI模型配置相关状态
  const [aiModelConfig, setAiModelConfig] = useState<AIModelConfig | null>(
    null
  );
  ...
  const configTabs = [
    ...
    { id: "ai-models", label: "AI模型配置" },
  ];

  // 保存AI模型配置
  const handleSaveAIModelConfig = async () => {
    if (!selectedKb || !aiModelConfig) {
      showToast("请先选择知识库", "warning");
      return;
    }

    if (!aiModelConfig.embedding_model?.trim()) {
      showToast("请输入Embedding模型名称", "warning");
      return;
    }
    if (!aiModelConfig.chat_model?.trim()) {
      showToast("请输入Chat模型名称", "warning");
      return;
    }

    try {
      setSavingAiConfig(true);
      await knowledgeBaseAPI.updateAIModelConfig(selectedKb.id, aiModelConfig);
      showToast("AI模型配置保存成功", "success");
    } catch (err: any) {
  ...
  const loadKnowledgeBaseDetail = async (kb: KnowledgeBase) => {
    try {
      ...
      // 加载AI模型配置
      try {
        const configResponse = await knowledgeBaseAPI.getAIModelConfig(kb.id);
        if (configResponse.data) {
          setAiModelConfig({
            embedding_provider:
              configResponse.data.embedding_provider || "ollama",
            embedding_model: configResponse.data.embedding_model || "",
            embedding_endpoint:
              configResponse.data.embedding_endpoint || undefined,
            chat_provider: configResponse.data.chat_provider || "ollama",
            chat_model: configResponse.data.chat_model || "",
            chat_endpoint: configResponse.data.chat_endpoint || undefined,
          });
        } else {
          setAiModelConfig({
            ...
            chat_provider: kb.chat_provider || "ollama",
  ...
  const handleSaveField = () => {
    ...
    // 检查字段名是否已存在（排除当前正在编辑的字段）
    // 使用字段名称比较而不是对象引用
    const existingFieldByName = schemaFields.find(
      (f) => f.name === editingField.name
    );
    const isDuplicate =
      existingFieldByName &&
      existingFieldByName.name !== editingField.name.trim();

    // 更简洁的重复检查：如果存在同名字段但不是当前编辑的字段
    const hasDuplicate = schemaFields.some(
      (f) => f.name === editingField.name && f.name !== editingField.name
    );
    ...
    // 检查是否是编辑现有字段（通过检查是否在schemaFields中存在同名字段）
    const isExistingField = schemaFields.some(
      (f) => f.name === editingField.name
    );

    // 如果是编辑现有字段，更新列表中的该字段
    if (isExistingField) {
      const updatedFields = schemaFields.map((f) =>
      ...
      <div className="flex h-full">
        ...
        <div className="flex-1 p-6 overflow-auto">
          <div className="space-y-6">
                          ...
                          .map((field) => (
                            ...
                            >
                              ...
                              <div className="space-y-1 text-xs text-gray-600">
                                ...
                                <div>
                                  • HNSW-ef: {field.hnsw?.ef_construct || 100}
                                </div>
                                <div>
                                  • 量化:{" "}
                                  {field.quantization?.type
                                    ? field.quantization.type.toUpperCase()
                                    : "无"}
                                </div>
                          ...
                          .map((field) => (
                            ...
                            >
                              ...
                              <div className="space-y-1 text-xs text-gray-600">
                                ...
                                <div>
                                  • 方法:{" "}
                                  {field.sparseMethod?.toUpperCase() || "BM25"}
                                </div>
                                <div>• 用途: 混合检索</div>
                              </div>
                            </div>
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <Input
                              autoComplete="off"
                              name={`${vectorDbType}-host-${
                                selectedKb?.id || "new"
                              }`}
                              id={`${vectorDbType}-host-${
                                selectedKb?.id || "new"
                              }`}
                            ...
                            <Input
                              type="number"
                              autoComplete="off"
                              name={`${vectorDbType}-port-${
                                selectedKb?.id || "new"
                              }`}
                              id={`${vectorDbType}-port-${
                                selectedKb?.id || "new"
                              }`}
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            placeholder={
                              vectorDbConfigModified.api_key
                                ? ""
                                : vectorDbConfigExists.api_key
                                ? "已配置（留空不修改，输入新值覆盖）"
                                : "API密钥（可选）"
                            }
                            value={
                              vectorDbConfigModified.api_key
                                ? vectorDbConfig.api_key || ""
                                : ""
                            }
                            onChange={(e) => {
                              setVectorDbConfig({
                    ...
                    {vectorDbType === "elasticsearch" && (
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <Input
                            autoComplete="off"
                            name={`${vectorDbType}-user-${
                              selectedKb?.id || "new"
                            }`}
                            id={`${vectorDbType}-user-${
                              selectedKb?.id || "new"
                            }`}
                            placeholder="用户名（可选）"
                          ...
                          <Input
                            type="password"
                            autoComplete="new-password"
                            name={`${vectorDbType}-password-${
                              selectedKb?.id || "new"
                            }`}
                            id={`${vectorDbType}-password-${
                              selectedKb?.id || "new"
                            }`}
                            placeholder={
                              vectorDbConfigModified.password
                                ? "密码（可选）"
                                : vectorDbConfigExists.password
                                ? "已配置（留空不修改，输入新值覆盖）"
                                : "密码（可选）"
                            }
                            value={
                              vectorDbConfigModified.password
                                ? vectorDbConfig.password || ""
                                : ""
                            }
                            onChange={(e) => {
                              setVectorDbConfig({
            ...
            {/* AI模型配置 */}
            {activeConfigTab === "ai-models" && aiModelConfig && (
              <div className="space-y-6">
                <div>
                  ...
                  <div className="border-b pb-6">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Select
                          value={aiModelConfig.embedding_provider || "ollama"}
                          onValueChange={(value) =>
                            setAiModelConfig({
                              ...aiModelConfig,
                      ...
                      <div>
                        ...
                        <Input
                          value={aiModelConfig.embedding_model || ""}
                          onChange={(e) =>
                            setAiModelConfig({
                              ...aiModelConfig,
                          ...
                          placeholder="例如：nomic-embed-text、bge-m3"
                        />
                      </div>

                      {aiModelConfig.embedding_provider === "ollama" && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Ollama服务地址（可选，留空使用默认配置）
                          </label>
                          <Input
                            value={aiModelConfig.embedding_endpoint || ""}
                            onChange={(e) =>
                              setAiModelConfig({
                                ...aiModelConfig,
                                embedding_endpoint: e.target.value || undefined,
                              })
                            }
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Select
                          value={aiModelConfig.chat_provider || "ollama"}
                          onValueChange={(value) =>
                            setAiModelConfig({
                              ...aiModelConfig,
                              chat_provider: value,
                      ...
                      <div>
                        ...
                        <Input
                          value={aiModelConfig.chat_model || ""}
                          onChange={(e) =>
                            setAiModelConfig({
                              ...aiModelConfig,
                      ...
                      {aiModelConfig.chat_provider === "ollama" && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Ollama服务地址（可选，留空使用默认配置）
                          </label>
                          <Input
                            value={aiModelConfig.chat_endpoint || ""}
                            onChange={(e) =>
                              setAiModelConfig({
                                ...aiModelConfig,
                                chat_endpoint: e.target.value || undefined,
                              })
                            }
                            placeholder="http://localhost:11434"
                          />
                ...
                <div className="flex justify-end gap-3">
                  <Button variant="outline" disabled={savingAiConfig}>
                    取消
                  </Button>
                  <Button
        ...
        {editingField && (
          ...
          >
            <DialogContent className="max-h-[90vh] flex flex-col max-w-2xl">
              <DialogHeader>
                <DialogTitle>
                  {schemaFields.includes(editingField)
                    ? "编辑字段"
                    : "添加字段"}
                </DialogTitle>
                <DialogDescription>
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="dense_vector">
                        稠密向量 (Named Vector)
                      </SelectItem>
                      <SelectItem value="sparse_vector">
                        稀疏向量 (Sparse Vector)
                      </SelectItem>
                    </SelectContent>
                  </Select>
                      ...
                      <div className="mt-3 p-3 border rounded space-y-2">
                        ...
                        <div>
                          ...
                          <Input
                            type="number"
                            value={
                              editingField.hnsw?.full_scan_threshold || 10000
                            }
                            onChange={(e) =>
                              setEditingField({
                                ...editingField,
                          ...
                          <div>
                            <label className="flex items-center space-x-2">
                              <input
                                type="checkbox"
                                checked={
                                  editingField.quantization?.always_ram || false
                                }
                                onChange={(e) =>
                                  setEditingField({
                                    ...editingField,
                  ...
                  {editingField.type === "sparse_vector" && (
                    <div className="mt-2">
                      ...
                      <Select
                        ...
                        onValueChange={(
                          value: "bm25" | "tf-idf" | "simple" | "splade"
                        ) =>
                          setEditingField({
                            ...editingField,
                            sparseMethod: value,
        ...
        {/* 加载Schema对话框 */}
        <Dialog
          open={loadSchemaDialogOpen}
          onOpenChange={setLoadSchemaDialogOpen}
        >
          <DialogContent>
            <DialogHeader>
                  ...
                  {(() => {
                    ...
                    if (selected?.metadata) {
                      return (
                        <div>
                          <div>
                            知识库: {selected.metadata.kb_name || "N/A"}
                          </div>
                          <div>
                            向量数据库:{" "}
                            {selected.metadata.vector_db_type || "N/A"}
                          </div>
                          <div>
                            字段数: {selected.metadata.field_count || "N/A"}
                          </div>
                        </div>
                      );
                    }
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setLoadSchemaDialogOpen(false)}
              >
                取消
              </Button>

...
export default function EvaluationHistoryView() {
  ...
  const handleViewCaseResult = async (resultId: string) => {
    ...
    try {
      const result = await evaluationAPI.getCaseResult(
        selectedTask.id,
        resultId
      );
  ...
  const handleCancelTask = async (taskId: string) => {
    if (!confirm("确定要终止这个评估任务吗？")) {
      return;
    }

    try {
      await evaluationAPI.cancelTask(taskId);
      showToast("评估任务已终止", "success");
      loadTasks(); // 刷新任务列表
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">查看历史评估任务和结果</p>
      </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center gap-4">
            ...
            <Select
              value={kbId || "all"}
              onValueChange={(value) => setKbId(value === "all" ? "" : value)}
            >
              <SelectTrigger className="w-[200px]">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="px-2 py-1 text-xs rounded bg-blue-100 text-blue-700">
                          {task.evaluation_type === "retrieval"
                            ? "检索"
                            : "生成"}
                        </span>
                        <span
                      ...
                      <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                        ...
                        <span>
                          进度: {task.completed_cases} / {task.total_cases}
                        </span>
                        <span>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    类型:{" "}
                    {selectedTask.evaluation_type === "retrieval"
                      ? "检索"
                      : "生成"}
                  </div>
                  ...
                  {summary.overall_retrieval_metrics && (
                    <div className="mb-2">
                      ...
                      <div className="grid grid-cols-4 gap-2 text-sm">
                        {Object.entries(summary.overall_retrieval_metrics).map(
                          ([key, value]) => (
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              {result.ragas_score !== undefined && (
                                <span>
                                  RAGAS: {(result.ragas_score * 100).toFixed(2)}
                                  %
                                </span>
                              )}
            ...
            <div className="space-y-4">
              ...
              {selectedResult.retrieval_metrics && (
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {Object.entries(selectedResult.retrieval_metrics).map(
                      ([key, value]) => (
                        <div
                          key={key}
              ...
              {selectedResult.ragas_retrieval_metrics && (
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {Object.entries(selectedResult.ragas_retrieval_metrics).map(
                      ([key, value]) => (
                        <div
                          key={key}
                          className="p-2 bg-gray-50 rounded text-sm"
